  ...
  describe ' #default usage' do
    ...
    it "should works" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tr", :count => (@users.size + 1))
        html.should have_tag("td", :count => (@users.size * (User.content_columns.size + 2)) + 1)
      end
    end
    ...
    it "should add link columns" do
      helper.sorting_table_for(@users) do |table|
        table.columns.should have_tag('a', :count => (@users.size * 2))
      end
    end
    ...
    it "should add i18n to link columns" do
      helper.sorting_table_for(@users) do |table|
        html = table.columns
        html.should have_tag('a', :text => 'Edit', :count => @users.size)
        html.should have_tag('a', :text => 'Delete', :count => @users.size)
      end
    end
    ...
    it "should add class odd/even" do
      helper.sorting_table_for(@users) do |table|
        html = table.columns
        html.should have_tag("tr[class=odd]", :count => (@users.size / 2))
        html.should have_tag("tr[class=even]", :count => (@users.size / 2))
      end
    end
    ...
    it "should customize html" do
      helper.sorting_table_for(@users, :html => { :class => 'table_class', :id => 'table_id' }) do |table|
        ...
        html.should have_tag("tr:nth-child(2)[class='hello_class odd']")
        html.should have_tag("tr:nth-child(2)[id=hello_id]")
        html.should have_tag("tr:nth-child(2)[title=hello_title]")
      end
  ...
  describe " #table columns" do
    ...
    it "should works with one column" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tbody", :count => 1)
        html.should have_tag("tr", :count => (@users.size + 1))
        html.should have_tag("td", :count => (@users.size + 1))        
      end
    end
    ...
    it "should have multi columns" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tbody", :count => 1)
        html.should have_tag("tr", :count => (@users.size + 1))
        html.should have_tag("td", :count => ((@users.size * 2) + 1))
      end
    end
    
    it "should have multi columns and action" do
      helper.sorting_table_for(@users) do |table|
        table.columns(:username, :price, :actions => :edit).should have_tag("td", :count => ((@users.size * 3) + 1))
      end      
    end

    it "should have multi columns and multi action" do
      helper.sorting_table_for(@users) do |table|
        table.columns(:username, :price, :actions => [:edit, :delete]).should have_tag("td", :count => ((@users.size * 4) + 1))
      end      
    end
    
    it "should do nothing if the key isn't correct" do
      helper.sorting_table_for(@users) do |table|
        table.columns(:username, :price, :blabla => :edit).should have_tag("td", :count => ((@users.size * 2) + 1))
      end
    end
    ...
    it "should works symbol and non symbol" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tr:nth-child(2) td:nth-child(1)", :text => @users.first.username)
        html.should have_tag("tr:nth-child(2) td:nth-child(2)", :text => 'hello')
        html.should have_tag("tr:nth-child(2) td:nth-child(3)") do
          with_tag "img"
        end
        html.should have_tag("tr:nth-child(2) td:nth-child(4)") do
          with_tag "a"
        end
      end
    end
    ...
    it "should works with non key symbol" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tr:nth-child(2) td:nth-child(1)", :text => @users.first.username)
        html.should have_tag("tr:nth-child(2) td:nth-child(2)", :text => '')
      end
    end    
    ...
    it "should add remote on link" do
      helper.sorting_table_for(@users, :link_remote => true) do |table|
        ...
        html.should have_tag("tr:nth-child(2) td:nth-child(2)") do
          if Tools::rails3?
            with_tag "a[remote]"
          else
            with_tag "a[onclick]"
    ...
    it "should customize html" do
      helper.sorting_table_for(@users, :html => { :class => 'table_class', :id => 'table_id' }) do |table|
        ...
        html.should have_tag("tr:nth-child(2)[class='hello_class odd'][id=hello_id][title=hello_title]")
      end
      helper.output_buffer.should have_tag("table[class='table_class sorting_table_for'][id=table_id]")
    end
  ...
  describe " #table column" do
    ...
    it "should works with multi types" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tr:nth-child(2) td:nth-child(1)", :text => @users.first.username)
        html.should have_tag("tr:nth-child(2) td:nth-child(2)", :text => '')
        html.should have_tag("tr:nth-child(2) td:nth-child(3)", :text => 'hello')
        html.should have_tag("tr:nth-child(2) td:nth-child(4)") do
          with_tag "img"
        end
        html.should have_tag("tr:nth-child(2) td:nth-child(5)") do
          with_tag "a"
        end
      end
    end
    ...
    it "should customize html" do
      helper.sorting_table_for(@users, :html => { :class => 'table_class', :id => 'table_id' }) do |table|
        ...
        html.should have_tag("tr:nth-child(2)[class='hello_class odd'][id=hello_id][title=hello_title]")
        html.should have_tag("tr:nth-child(2) td:nth-child(1)[class='hi_class'][id=hi_id][title=hi_title]")
      end
      helper.output_buffer.should have_tag("table[class='table_class sorting_table_for'][id=table_id]")
    end
  ...
  describe " #table column with value" do
    ...
    it "should have symbol and value" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag("tr:nth-child(2) td:nth-child(1)", :text => number_to_currency(@users.first.price))
        html.should have_tag("tr:nth-child(2) td:nth-child(2)", :text => (@users.first.price - 1))
      end
    end
  ...
  describe " #table sub column" do
    ...
    it "should not change symbol" do
      helper.sorting_table_for(@users) do |table|
        ...
        html.should have_tag('tr:nth-child(2) td:nth-child(1)', :text => 'username')
        html.should have_tag('tr:nth-child(2) td:nth-child(2)', :text => 'hello')
      end
    end
  ...
  describe ' #With editable options' do
    ...
    it "should change reserved columns" do
      ...
      helper.sorting_table_for(@users) do |table|
        html = table.columns
        html.should have_tag("tr", :count => (@users.size + 1))
        html.should have_tag("td", :count => ((@users.size * 3) + 1))
      end
    end
    ...
    it "should edit default actions" do
      ...
      helper.sorting_table_for(@users) do |table|
        table.columns.should have_tag("td", :count => (@users.size * (User.content_columns.size + 4)) + 1)
      end
    end
